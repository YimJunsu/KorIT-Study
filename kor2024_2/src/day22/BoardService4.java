package day22;
      /*
         BoardService4
            ...
            - 수정조건 : 고정길이가 아닌 가변길이 형식으로 수정하여 100개 아닌 무한개 저장 가능한 가변길이 배열 만들기
            * 이유: 가변길이의 여러개 데이터를 관리할 때는 -> 컬렉션프레인워크(ArrayList) = 실무
                                                    - 가변배열 = 시험/코딩테스트
       */
            ...
            if (choose == 1) {
                ...
                if(count != 1) { //만약 기존 배열내 게시물이 존재하면
                    for(int i = 0; i <= boardList.length - 1; i++) {
                        newBoardList[i] = boardList[i]; // 기존배열내 게시물들을 새로운배열에 대입 / 카피`복사
                    }
                }
