...
/*
    boardService9mvc
                ...
                [생각해보기]
                1. txt 메모장은 문자열만 저장되는 윈도우 프로그램의 확장자, txt(객체지향X)
                2. 게시물들( ArrayList<BoardDto> ) 저장을 하기 위해서는 변환이 필요하다. jave(객체지향 o)
                물제점 발견 : 서로 다른 언어/프로그램 간의 데이터를 주고 받을려면 형식/타입이 같으면 좋을텐데.
                ...
                1. boardDto(content"안녕하세요", writer="유재석", pwd=1234) : 자바객체를 임의로 시각화
...
public class BoardDao {
    ...
    public boolean boardWrite(BoardDto boardDto) {
        ...
        filesave(); //만약에 리스트에 게시물을 추가 했다면, 파일에도 추가된 게시물을 쓰기
        return true;
    }
    ...
    public void filesave() { // ( 게시물 등록을 성공했을때 지정한 함수 사용/호출)
        ...
        for (int index = 0; index <= boardDB.size() - 1; index++) { // -리스트 객체 내 0번 인덱스부터 마지막 인덱스까지 반복
            ...
            // += 복합대입 연산자 : 오른쪽 값을 왼쪽변수 값과 더한 후에 결과를 왼쪽변수에 대입한다.
            // 5. [ 객체 구분 ]
            outStr += "\n";
        }
        System.out.println(outStr); // 확인용

        // try{}-catch(){} : try{} 예상치 못한 예외가 발생 했을때 지정된 catch 코드로 흐름을 이용하는 문법
        try {
            // [1] 파일 출력 객체 생성한다.
            FileOutputStream outputStream = new FileOutputStream("./src/day25/boardservice9mvc/data.txt");
            // [2] 파일 출력 객체를 이용한 바이트 쓰기/내보내기, 여러개
    ...
    public void fileLoad() {
        try {
            ...
            //활용과제 : 파일로 부터 읽어온 문자열의 게시물 정보들을 다시 DB에 저장하시오
        } catch (FileNotFoundException e) {
            e.printStackTrace();}
        catch (IOException e) {
            e.printStackTrace();
