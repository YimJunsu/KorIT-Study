...
#[1] 테이블 조회,
select * from member; # 'member' 테이블의 모든 필드 표시하는 모든 레코드 조회
select * from buy;	  # 'buy' 테이블의 모든 필드 표시하는 모든 레코드 조회
#[2] 특정 필드의 테이블 조회 
...
select mid as 회원아이디 from member; #'member' 테이블의 mid 필드를 '회원아이디로'로 표시하는 모든 레코드 조회
select mid 회원아이디 from member m; #'member' 테이블의 mid 필드를 '회원아이디'로 표시하는 모든 레코드 조회
#[4] 조건절, where 조건절 
select * from member where mname = '블랙핑크'; #'member'테이블의 모든 필드 표시하는 mname이 블랙필크인 레코드만 조회
select * from member where mnumber = 4; #'member' 테이블의 모든 필드 표시하는 mnumber가 4인 레코드만 조회
select * from member where mheight <= 162; #'member' 테이블의 mheight 필드값이 162 이하인 레코드만 조회
select * from member where mheight >= 165 and mheight <= 170; # 'mheight' 값이 165이상 이면서 170이하인 레코드 조회
select * from member where mheight between 165 and 170; # 165이상 170이하인 레코드조회
select * from member where mheight >= 165 or mnumber > 6; # 'mheight' 필드값이 165이상 이거나 'mnumber'의 값이 6초과인 레코드 조회
...
select * from member where mname != '에이핑크'; # 'mname'의 필드값이 에이핑크가 아닌 레코드 조회
select * from member where not mname = '에이핑크';
select * from member where mname like '에이%'; # 'mname' 필드값이 에이로 시작하는 레코드
select * from member where mname like '%핑크'; # 'mname' 값이 핑크로 끝나는 레코드
select * from member where mname like '%이%'; # 'mname' 값이 '이'가 포함된 레코드 조회
select * from member where mname like '_이_'; # 필드값이 '이'두번째 글자인 세글자 문자열 레코드 조회
...
연산자
	...
	2. 관계연산자
            ...
            3) 부정/반대alter		 : not 조건, 조건 결과의 반대
	3. 기타연산자
			1) 필드명 between 시작값 and 끝값 : 지정한 필드명의 값이 시작값과 끝값 사이이면 true
            2) 필드명 in(값1,값2,값3)		  : 지정한 필드명의 값이 in안에 있는 값이 하나라도 존재하면 true
            3) 필드명 like 패턴 : 지정한 필드명의 값이 패턴과 일치하면 true
				- 패턴 
				1. % : 모든 문자수를 대응 		ex) 김% -> 김으로 시작하는 문자열
                2. _ : _개수만큼 문자수 대응 	ex) 김_: 김으로 시작하는 두글자인 문자열
			4) 필드명 is null : 지정한 필드명의 값이 null 이면 true
            5) 필드명 is not null : 지정한 필드명의 값이 null이 아니면 true
	4. 산술연산자
		필드명 + 값 : 지정한 필드값에 값 더하기
        필드명 - 값 : 지정한 필드값에 값 빼기
        필드명 * 값 : 지정한 필드값에 값 곱하기
        필드명 / 값 : 지정한 필드값에 값 나누기
        필드명 div 값 : 지정한 필드값에 값를 나눈 몫
        필드명 mod 값 : 지정한 필드값에 값를 나눈 나머지
	5. 키워드
        ...
        3. having 그룹조건	: select 필드명 from 테이블명 where 일반조건 
...
# order by 필드명 asc(생략가능) : 지정한 필드명 기준으로 레코드를 오름차순 정렬, 작은수에서 큰수 / 과거날짜에서 최근날짜
# order by 필드명 desc : 지정한 필드명 기준으로 레코드를 내림차순 정렬, 큰수에서 작은수 / 최근날짜에서 과거날짜
# order by 필드명1 asc/desc, 필드명2 asc/desc : 정렬기준이 2개 이상일때, (쉼표)로 구분하기
...
#limit 레코드수
#limit 시작레코드번호(0~), 개수
select * from member limit 2; # 조회결과의 레코드를 위에서부터 2개만 조회 제한
select * from member limit 0, 3; #조회결과의 레코드에서 0(첫번째)레코드 부터 3개만 조회 제한
select * from member limit 2, 2; #조회결과의 레코드에서 2(세번째)레코드 부터 2개만 조회 제한
select * from member order by mheight desc limit 3; # 'mheight' 필드 기준으로 내림차순 후 3개만 조회 = 상위 3개

#[7] 필드값이 중복인 레코드 제거alter
# distinct
select maddr from member; # 주소 필드 모든 레코드를 조회
select distinct maddr, mdebut from member; #주소 필드의 모든 레코드를 조회하는데 주소가 중복이면 중복된 레코드는 제거
...
select * from member where maddr = '서울' order by mdebut desc limit 0, 2; # 주소 -> 서울, 데뷔일 기준 내림차순 후 첫번째부터 2개만 조회
...
select count(bgname) from buy; # 모든 레코드의 bgname(그룹명) 구매수량 데이터 개수, null 값을 제외한 필드 레코드 수
...
#구매수량이 2개 초과인 레코드에서 회원아이디별 구매금액 평균이 50이상인 레코드 조회
...
#구매수량이 2개 초과인 레코드에서 회원아이디별 구매금액 평균이 50이상인 레코드 조회, 회원아이디별 내림차순 정렬 후 첫번재 레코드에서 1개만 제한 조회
