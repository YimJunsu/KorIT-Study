...
public class BoardDao {
    ...
    public ArrayList<BoardDto> boardPrint( ) {
        ArrayList<BoardDto> list = new ArrayList<>(); // -- 조회된 레코드들을 객체화 해서 저장할 리스트 객체 선언
        try {
            //1. SQL 작성
            String sql = "select * from board";
            //2. SQL 기재
            PreparedStatement ps = conn.prepareStatement(sql); //기재할 SQL준비
            //3. SQL 조작, sql 실행 결과 조회된 SQL 결과 조작하는 resultSet 객체 반환
            ResultSet rs = ps.executeQuery();
            //4. SQL 실행

            //5. SQL 결과
            while (rs.next()) { // while(조건){} : 반복문, rs next() : 조회 결과에서 다음 레코드로 이동함수
                // 만약 레코드가 3개 존재 -> rs.next()는 3번 실행
                //[해석] 조회 결과 첫번째 레코드부터 마지막레코드까지 하나씩 레코드 이동
                //6. 레코드를 읽어서 각 필드별 데이터 호출, rs.getXXX("필드명") : 지정한 필드명의 값 타입에 맞게 값 변환
                int num = rs.getInt("num");           //현재 조회중인 레코드 게시물 번호(num)필드 값 호출
                String content = rs.getString("content");
                String writer = rs.getString("writer");
                int pwd = rs.getInt("pwd");
                //7. 각 레코드의 호출된 필드값들을 초기화 -> Dto 생성
                BoardDto boardDto = new BoardDto(content, writer, pwd);
                //8. 1개 레코드를 DTO 객체로 변환된 DTO를 리스트에 저장
                list.add(boardDto);
            }   // - 반복문 1번에 레코드 1개를 dto로 변환
            /* 예) 조회결과
                num     content     writer      pwd     <-- rs : 인터페이스가 조작할 수 있다. rs.next(): 다음레코드이동
                1       자바에서작성  유재석        1234
                2       안녕db        강호동     4567
                3       하하하하하      하하       7897
